...
def vectorize(ex, word2ind):
    ...
    for x in range(len(question_text)):
        if question_text[x] in word2ind:
            vec_text[x] = word2ind[question_text[x]]
        else:
            vec_text[x] = word2ind["<unk>"]
...
class DanModel(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_dim=50,
        ...
        self.classifer = nn.Sequential(self.linear1, nn.ReLU(), nn.Dropout(self.nn_dropout), self.linear2)
